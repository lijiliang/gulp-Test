/*==================================================
 ...
 smcore.mobile.shim.js 1.4.7.1 built in 2015.10.30
 ==================================================*/
(function(global, factory) {
    ...
    head.insertAdjacentHTML("afterBegin", '<smcore vm-skip class="smcoreHide"><style id="smcoreStyle">.smcoreHide{ display: none!important }</style></smcore>')
    var ifGroup = head.firstChild

    function log() {
        if (smcore.config.debug) {
    ...
    var smcoreFragment = DOC.createDocumentFragment()
    ...
    smcore.profile = function() {
        if (window.console && smcore.config.profile) {
    ...
    smcore.contains = function(root, el) {
        try {
        ...
        names.forEach(function(name, accessor) {
                ...
                if (isObservable(name, val, $skipArray)) {
                    ...
                    if (valueType === "object" && isFunction(val.get) && Object.keys(val).length <= 2) {
                        ...
                        // issue #940 解决$model层次依赖丢失 https://github.com/RubyLouvre/smcore/issues/940
        ...
        function accessor(value) {
            ...
            if (arguments.length > 0) {
                ...
                if (valueType === "array") {
                    ...
                    var newObject = smcore.mix(true, {}, value)
                    for (i in son) {
                    ...
                    if (observes.length) {
                        observes.forEach(function(data) {
                            ...
                            if (data.rollback) {
                                data.rollback() //还原 vm-with vm-on
                            }
    ...
    function arrayFactory(model) {
        ...
        smcore.mix(array, arrayPrototype)
        return array
    }
    ...
    smcore.injectBinding = function(data) {
        ...
        if (valueFn) { //如果是求值函数
            ...
            try {
                ...
                log("warning:exception throwed in [smcore.injectBinding] ", e)
                delete data.evaluator
    ...
    function injectDependency(list, data) {
        if (data.oneTime)
            return
        if (list && smcore.Array.ensure(list, data) && data.element) {
     ...
     **********************************************************************/
    var disposeCount = 0
    var disposeQueue = smcore.$$subscribers = []
    ...
    function injectDisposeQueue(data, list) {
        ...
        smcore.Array.ensure(lists, list)
            ...
            while (data = disposeQueue[--i]) {
                ...
                if (iffishTypes[data.type] && shouldDispose(data.element)) { //如果它没有在DOM树
                    ...
                    for (var k = 0, list; list = lists[k++];) {
                        smcore.Array.remove(lists, list)
                        smcore.Array.remove(list, data)
                    }
                    disposeData(data)
                }
            }
    ...
    function shouldDispose(el) {
        ...
        return el.msRetain ? 0 : (el.nodeType === 1 ? !root.contains(el) : !smcore.contains(root, el))
    }
    ...
    smcore.parseHTML = function(html) {
        var fragment = smcoreFragment.cloneNode(false)
    ...
    if (root.dataset) {
        smcore.fn.data = function(name, val) {
    ...
    //=============================css相关==================================
    var cssHooks = smcore.cssHooks = createMap()
    var prefixes = ["", "-webkit-", "-moz-", "-ms-"] //去掉opera-15的支持
    var cssMap = {
        "float": "cssFloat"
    }
    smcore.cssNumber = oneObject("animationIterationCount,columnCount,order,flex,flexGrow,flexShrink,fillOpacity,fontWeight,lineHeight,opacity,orphans,widows,zIndex,zoom")
            ...
            data.vars.forEach(function(v) {
                ...
                code = code.replace(reg, function(_, cap) {
                    var c = _.charAt(v.length)
                        ...
                        //https://github.com/RubyLouvre/smcore/issues/966
        ...
        if (/\S/.test(filters)) { //文本绑定，双工绑定才有过滤器
            ...
            try {
                ...
                data.evaluator = evaluatorPool.put(exprId, function() {
                    return fn.apply(smcore, arguments) //确保可以在编译代码中使用this获取smcore对象
                })
            } catch (e) {
    ...
    smcore.scan = function(elem, vmodel) {
        elem = elem || root
        ...
        if (vmodels.length) {
            ...
            if (bindings.length) {
                ...
                if (control && hasDuplex) {
                    if (msData["vm-attr-value"] && elem.type === "text") {
                        log("warning!" + control + "控件不能同时定义vm-attr-value与" + hasDuplex)
                    }
                }
    ...
    function scanNodeList(parent, vmodels) {
        var nodes = smcore.slice(parent.childNodes)
    ...
    function scanText(textNode, vmodels) {
        ...
        if (tokens.length) {
            for (var i = 0, token; token = tokens[i++];) {
                ...
                smcoreFragment.appendChild(node)
            }
            textNode.parentNode.replaceChild(smcoreFragment, textNode)
            if (bindings.length)
                executeBindings(bindings, vmodels)
        }
    }
    ...
    var templatePool = smcore.templateCache = {}
    ...
    new function() { // jshint ignore:line
        try { //#272 IE9-IE11, firefox
            ...
            function newSetter(value) { // jshint ignore:line
                ...
                if (!this.msFocus && this.smcoreSetter && this.oldValue !== value) {
                    this.smcoreSetter()
                }
            }
    ...
    bindingExecutors.html = function(val, elem, data) {
        ...
        if (typeof val !== "object") { //string, number, boolean
            var fragment = smcore.parseHTML(String(val))
        } else if (val.nodeType === 11) { //将val转换为文档碎片
            fragment = val
        } else if (val.nodeType === 1 || val.item) {
            var nodes = val.nodeType === 1 ? val.childNodes : val.item
            fragment = smcoreFragment.cloneNode(true)
            while (nodes[0]) {
        ...
        if (isHtmlFilter) {
            ...
            smcore.clearHTML(elem).appendChild(fragment)
        }
    ...
    bindingHandlers.repeat = function(data, vmodels) {
        ...
        if (elem.nodeType === 1) {
            ...
            data.template = smcoreFragment.cloneNode(false)
            if (type === "repeat") {
    ...
    bindingExecutors.repeat = function(method, pos, el) {
        ...
        if (method) {
            ...
            var transation = smcoreFragment.cloneNode(false)
            switch (method) {
    ...
    bindingExecutors.visible = function(val, elem, binding) {
        if (val) {
            ...
            if (smcore(elem).css("display") === "none") {
    ...
    new function() { // jshint ignore:line
        ...
        var gestureHooks = smcore.gestureHooks = {
            pointers: {},
